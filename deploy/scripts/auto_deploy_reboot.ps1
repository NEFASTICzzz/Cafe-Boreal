...
# 1. Montar praEXAM en ~/praEXAM
Send-CommandDirect "sudo mkdir -p ~/praEXAM && sudo mount -t vboxsf praEXAM ~/praEXAM && cd ~/praEXAM"
Start-Sleep -Seconds 2
Send-CommandDirect "1234"
Start-Sleep -Seconds 2

# 2. Correr deploy one-click
Send-CommandDirect "sudo bash deploy/scripts/deploy_one_click.sh"
Start-Sleep -Seconds 2
Send-CommandDirect "1234"
